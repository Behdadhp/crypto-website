...
class PortfolioList(generic.ListView , LoginRequiredMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        queryset_portfoilio = models.Portfolio.objects.filter(user_id = self.request.user)
        queryset_market = models.Market.objects.all()
        queryset_user = models.User.objects.all()

        portfolio=[]

        for item in queryset_portfoilio:
    ...
    #  Total Asset of each coin:
        ...
        for i in range(len(unique)):
            ...
            for j in range(len(portfolio)):
                if unique[i] == portfolio[j]['type']:
                    if portfolio[j]['status'] == 'Buy':
                        asset += portfolio[j]['amount']
                    elif portfolio[j]['status'] == 'Sell':
                        asset -= portfolio[j]['amount']
            for x in data:
                if x['name'] == unique[i]:
...
# End of Total Asset of each Coin

# Value of each coin in DetailView

        for i in queryset_market.values():
            if coin == i['id']:
                each_coin = i['coin']
...
# end of value of each on in DetailView
        return context
